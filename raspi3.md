...
| `meteo.tongatron.org` | `localhost:3505` | meteo-piemonte |
| `bandotorino.tongatron.org` | `127.0.0.1:3506` | bandotorino |
| `esp32-c3.tongatron.org` | `127.0.0.1:3507` | objects3D (mostra XIAO ESP32-C3) |
...
| `3505` | `127.0.0.1` | `meteo-piemonte.service` | meteo-piemonte |
| `3506` | `127.0.0.1` | `bandotorino.service` | bandotorino |
| `3507` | `127.0.0.1` | `objects3d.service` | objects3D (statico, python http.server) |
...
- **Service:** `objects3d.service`
- **Working dir:** `/srv/apps/objects3D`
- **Git:** `https://github.com/tongatron/objects3D.git` (pubblico, branch `main`)
- **Avvio:** `/usr/bin/python3 -m http.server 3507 --bind 127.0.0.1 --directory /srv/apps/objects3D/dist`
- **Bind:** `127.0.0.1:3507`
- **Hostname pubblico:** `https://esp32-c3.tongatron.org`
- **systemd:** `MemoryMax=100M`

Scena 3D interattiva (Three.js/Vite) con la mostra del XIAO ESP32-C3: scheda in scala 10:1
su piedistallo, componenti cliccabili con descrizioni e vista esplosa. Sito statico:
la build sta in `dist/`, servita da `python3 -m http.server` (nessuna dipendenza runtime).
La stessa app è anche su GitHub Pages come `https://tongatron.org/objects3D/` (base path diverso).

Aggiornamento: `cd /srv/apps/objects3D && git pull && npx vite build --base=/ && sudo systemctl restart objects3d`
(riavvio non strettamente necessario: il server legge i file da disco).

---

### meteo-piemonte

- **Service:** `meteo-piemonte.service`
- **Working dir:** `/srv/apps/meteo-piemonte`
- **Avvio:** `/usr/bin/node /srv/apps/meteo-piemonte/server.js`
- **Bind:** `127.0.0.1:3505`
- **Hostname pubblico:** `https://meteo.tongatron.org`

---

### bandotorino

- **Service:** `bandotorino.service`
- **Working dir:** `/srv/apps/bandotorino`
- **Avvio:** `/usr/bin/node /srv/apps/bandotorino/server.js`
- **Bind:** `127.0.0.1:3506`
- **Hostname pubblico:** `https://bandotorino.tongatron.org`
