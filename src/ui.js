...
export function createUI({ state, setObject, setFloor, neon, ambient, bulb, insects, setVista, wind, daisies, roses, grass, monsters, mower, bamboo, panda, xiao, nuraghe, share }) {
  ...
  // --- scena ---
  gui.add(state, 'oggetto', ['Girasoli', 'Campo di margherite', 'Campo di rose rosse', 'Foresta di bambù', 'Orsetto peluche', 'Nuraghe sardo', 'XIAO ESP32-C3'])
    .name('Oggetto')
    .onChange(setObject);
  ...
  // --- nuraghe sardo ---
  const nuragheFolder = gui.addFolder('Nuraghe sardo');
  nuragheFolder.add(nuraghe.params, 'popcorn').name('Popcorn');
  nuragheFolder.add(nuraghe.params, 'intensita', 0.1, 4, 0.05).name('Intensità popcorn');
