...
export function createTeddy() {
  ...
  // corpo seduto
  const body = add(new THREE.Mesh(new THREE.SphereGeometry(0.62, 64, 48), furMat));
  body.position.y = 0.62;
  body.scale.set(1, 1.15, 0.9);

  // pancia chiara
  const belly = add(new THREE.Mesh(new THREE.SphereGeometry(0.45, 56, 40), muzzleMat));
  belly.position.set(0, 0.62, 0.24);
  belly.scale.set(0.85, 1.0, 0.7);

  // testa
  const head = add(new THREE.Mesh(new THREE.SphereGeometry(0.48, 64, 48), furMat));
  head.position.y = 1.55;

  // muso
  const muzzle = add(new THREE.Mesh(new THREE.SphereGeometry(0.22, 48, 32), muzzleMat));
  muzzle.position.set(0, 1.44, 0.38);
  muzzle.scale.set(1.1, 0.8, 0.9);

  // naso
  const nose = add(new THREE.Mesh(new THREE.SphereGeometry(0.07, 32, 24), darkMat));
  nose.position.set(0, 1.5, 0.57);
  nose.scale.set(1.3, 0.8, 0.8);

  // occhi
  for (const s of [1, -1]) {
    const eye = add(new THREE.Mesh(new THREE.SphereGeometry(0.05, 28, 20), darkMat));
    eye.position.set(s * 0.17, 1.66, 0.4);
  }

  // orecchie
  for (const s of [1, -1]) {
    const ear = add(new THREE.Mesh(new THREE.SphereGeometry(0.16, 44, 32), furMat));
    ear.position.set(s * 0.34, 1.94, 0);
    ear.scale.set(1, 1, 0.55);
    const inner = add(new THREE.Mesh(new THREE.SphereGeometry(0.08, 32, 24), muzzleMat));
  ...
  // braccia aperte
  for (const s of [1, -1]) {
    const arm = add(new THREE.Mesh(new THREE.CapsuleGeometry(0.15, 0.42, 24, 48), furMat));
    arm.position.set(s * 0.62, 0.95, 0.1);
    arm.rotation.z = s * 1.15;
    arm.rotation.x = -0.25;
    const paw = add(new THREE.Mesh(new THREE.SphereGeometry(0.13, 40, 28), muzzleMat));
    paw.position.set(s * 0.92, 1.12, 0.2);
  }

  // gambe distese in avanti (seduto)
  for (const s of [1, -1]) {
    const leg = add(new THREE.Mesh(new THREE.CapsuleGeometry(0.17, 0.4, 24, 48), furMat));
    leg.position.set(s * 0.32, 0.22, 0.45);
    leg.rotation.x = Math.PI / 2.15;
    const foot = add(new THREE.Mesh(new THREE.SphereGeometry(0.17, 44, 32), muzzleMat));
  ...
  const stitch = add(new THREE.Mesh(new THREE.TorusGeometry(0.48, 0.012, 12, 96, Math.PI * 0.7), stitchMat));
